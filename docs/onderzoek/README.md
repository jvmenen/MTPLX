...
| 2026-09-26 | [batching](2026-09-26-batching.md) | Batching samen met MTP: welke modi er zijn en waarom `mtp_batch` weinig oplevert voor Bink |
...
| 2026-09-26 | [eindbenchmark](2026-09-26-eindbenchmark.md) | 2.11.3 tegen de integratietak (en schone 2.12.0) op het echte model: agentgesprek −64%, scoreroute −17% bitgelijk, eerste-token-logprobs −37% zonder completions-bank, decode +6% van upstream; toewijzing per fix. Eindtest 2 (avond, 49c4a6ee met invariante prefill, zonder bank en 128-drempel): scoreroute −36%, eerste-token-logprobs −39%, agent −66%; nieuwe aanbeveling voor de overstap |
...
| `perf/integratie` | Alle fixes samen voor de eindbenchmark | Alleen lokaal (`~/Dev/MTPLX-integratie`, nu `f26dc9a8` na fast-forward naar `perf/invariant-lane-moe-only`); pushen werd geweigerd (vondst 48). Eindbenchmark gedraaid (26 sep, [eindbenchmark](2026-09-26-eindbenchmark.md)), eindtest 2 met de lane (aanbevolen voor de overstap); gebruikt door het Bink-platform sinds de overstap (vondst 46) |
...
| `perf/invariant-lane-gate` | Lane alleen installeren als hij elke projectie dekt (weigert o.a. Bonsai: `unsupported_linear:HadamardQuantizedLinear`); reden in `/health` | Gepusht (49c4a6ee); in `perf/integratie` (fast-forward); op A3B op de server gemeten in eindtest 2 (lane geïnstalleerd, 30 GDN-lagen, [eindbenchmark](2026-09-26-eindbenchmark.md)); op dichte modellen kost de lane iets (vondst 59) |
| `perf/invariant-lane-moe-only` | Lane alleen op MoE-modellen (vondst 59): dicht model geweigerd met `dense_model`, knop `MTPLX_BATCH_INVARIANT_PREFILL_DENSE=1` om hem toch te installeren; op `perf/invariant-lane-gate` | Gepusht (f26dc9a8); in `perf/integratie` (fast-forward); dit is de tak die het Bink-platform via de overstap gebruikt (vondst 46) |
...
| `pr/messages-ttft` | PR-tak voor `fix/messages-ttft` (vondst 43): kale goedgevormde toolaanroep niet meer als orphan markup herhalen | Gepusht; PR [#539](https://github.com/youssofal/MTPLX/pull/539) ingediend |
| `pr/completions-overhead` | PR-tak voor `fix/completions-overhead` (vondst 42): geen blank retries bij temperatuur 0 | Gepusht; PR [#540](https://github.com/youssofal/MTPLX/pull/540) ingediend |
| `pr/gemma4-probe` | PR-tak voor `fix/gemma4-probe-vocab` (vondst 42): Gemma 4-controle zonder `get_vocab()` | Gepusht; PR [#541](https://github.com/youssofal/MTPLX/pull/541) ingediend |
| `pr/chat-scoped-segments` | PR-tak voor `perf/chat-scoped-segments` (vondst 50): scoped chat per beurt encoderen | Gepusht; PR [#542](https://github.com/youssofal/MTPLX/pull/542) ingediend (depends on #533) |
| `pr/first-token-logprobs-gemma4` | PR-tak voor `feat/first-token-logprobs-gemma4`: eerste-token-logprobs op Gemma 4 | Gepusht; PR [#543](https://github.com/youssofal/MTPLX/pull/543) ingediend (depends on #530) |
