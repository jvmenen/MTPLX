...
| `perf/integratie` | Alle fixes samen voor de eindbenchmark | Alleen lokaal (`~/Dev/MTPLX-integratie`, nu 49c4a6ee na fast-forward naar `perf/invariant-lane-gate`); pushen werd geweigerd (vondst 48). Eindbenchmark gedraaid (26 sep, [eindbenchmark](2026-09-26-eindbenchmark.md)), eindtest 2 op 49c4a6ee met de lane (aanbevolen voor de overstap); 49c4a6ee staat op de fork als `perf/invariant-lane-gate`; modeltest op 198e7194 ([modeltest](2026-09-26-modeltest.md)) |
...
| `perf/invariant-lane-moe-only` | Lane alleen op MoE-modellen (vondst 59): dicht model geweigerd met `dense_model`, knop `MTPLX_BATCH_INVARIANT_PREFILL_DENSE=1` om hem toch te installeren; op `perf/invariant-lane-gate` | Gepusht (f26dc9a8); tests groen; op de server nog te toetsen op q9b; niet in `perf/integratie` |
